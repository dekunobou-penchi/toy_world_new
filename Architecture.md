...
本プロジェクトは、研究用 Web アプリケーション（グリッドワールド型のマルチプレイヤー実験プラットフォーム）の再構築である。
旧版は PHP で実装されていたが、以下の理由により全面的に作り直す。

- 保守性が悪い
- ゲーム的なアプリケーションに PHP は不向き
- より扱いやすいシステムにしたい
- 現行アプリの同期方式が今後の構想に合わない

トーラス型のグリッドマップ上で、プレイヤー（人間および将来的には機械学習エージェント）がブロックの収集・設置・摂取・融合を行いながら生存する。部分観測（前方の限られた範囲のみ視認可能）が研究設計上の重要な要素となっている。
...
エージェント（Python）との型共有は後回し（プロトコル確定済みのため、後から docs / JSON Schema 化が可能）。
